...
Normal access-log credential redaction is covered now. Diagnostic-bundle redaction will be tested when the phase 17 diagnostic exporter exists. The release matrix must also repeat discovery on a normal LAN and compatibility checks against the selected release-time client versions.
