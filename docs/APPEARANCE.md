...
Named watchlists, their settings and membership are private server data. Watch Later is created when a user first accesses their lists or saves a video. Membership in any list protects downloads from retention cleanup. Manual reordering applies immediately and rolls back with an error if the server rejects it. Private WebSocket events update other connected sessions. Card shortcuts, feed filters, watchlist expansion/selection and browser playback volume persist with account appearance preferences and synchronize between clients of the same user. Each device remembers its last page and Settings tab for that server and user.
