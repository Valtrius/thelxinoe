...
A TV season needs fresh Sonarr series metadata, an exact complete set of confirmed Sonarr episode mappings, known past air dates, and evidence that the season finished: an ended/cancelled series, a later aired season, or a Sonarr-marked season/series finale. Missing, stale, complex or uncertain mappings block cleanup. Specials are excluded by default. Metadata freshness is seven days; dates must precede the current UTC date.
