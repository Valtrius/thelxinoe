...
pub async fn resolve(
    ...
    state.db.call(move|db|{
        let tx=db.transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)?;
        let principal=tx.query_row("SELECT u.id,u.username,u.role,u.timezone,s.id,s.transport FROM playback_grants g JOIN sessions s ON s.id=g.session_id JOIN users u ON u.id=g.user_id WHERE g.token_hash=?1 AND g.resource=?2 AND g.expires_at>?3 AND s.expires_at>?3",params![hash,resource,now()],|r|Ok(Principal{user:thelxinoe_auth::user_row(r)?,session_id:r.get(4)?,transport:r.get(5)?})).optional()?;
        if consume{tx.execute("DELETE FROM playback_grants WHERE token_hash=?1",[hash])?;}tx.commit()?;Ok(principal)
    }).await
}
pub async fn event_ticket(
    ...
    Ok(Json(json!({"ticket":issue(&state,&p,"events",30).await?})))
}
