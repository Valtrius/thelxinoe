CREATE TABLE live_media (id TEXT PRIMARY KEY, title TEXT NOT NULL);
CREATE TABLE playback_sessions_next (
    ...
    CHECK((media_id IS NOT NULL AND file_id IS NOT NULL AND youtube_video_id IS NULL AND live_media_id IS NULL) OR
          (media_id IS NULL AND file_id IS NULL AND youtube_video_id IS NOT NULL AND live_media_id IS NULL) OR
          (media_id IS NULL AND file_id IS NULL AND youtube_video_id IS NULL AND live_media_id IS NOT NULL AND streaming=1))
);
...
CREATE TABLE live_history (
 ...
 user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
 media_id TEXT NOT NULL, title TEXT NOT NULL, device_name TEXT NOT NULL,
 started_at INTEGER NOT NULL, updated_at INTEGER NOT NULL,
 position REAL NOT NULL, played_seconds REAL NOT NULL DEFAULT 0, state TEXT NOT NULL
);
CREATE INDEX live_history_user ON live_history(user_id,started_at DESC);
